...
use crate::{
    id,
    tool::{ToolContext, ToolCreationError},
    PhysicalPosition,
};
...
pub struct RotateTool {
    ...
    pub center_pos_screen: PhysicalPosition<f32>,
...
impl RotateTool {
    pub fn new(state: &crate::State, ctx: ToolContext) -> Result<Self, ToolCreationError> {
        // Calculate the average unfolded world position of the selected elements
        ...
        let x = state.settings.viewport_split_x * ctx.viewport.width;
        let y = (1.0 - state.settings.viewport_split_y) * ctx.viewport.height;
        let width = ctx.viewport.width - x;
        let height = ctx.viewport.height - y;
        let view_proj = state.viewport_2d.camera.view_proj(width, height);
        let center_pos_ndc = view_proj.transform_point(center_pos_world);
        let center_pos_screen = PhysicalPosition {
            x: (x + (width * (0.5 + center_pos_ndc.x / 2.0))) / ctx.dpr,
            y: (y + (height * (1.0 - (0.5 + center_pos_ndc.y / 2.0)))) / ctx.dpr,
        };
