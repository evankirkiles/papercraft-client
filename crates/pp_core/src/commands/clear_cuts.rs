...
pub struct ClearCutsCommand {
    pub edges: Vec<(MeshId, id::EdgeId)>,
}

impl ClearCutsCommand {
        ...
        let cmd = Self {
            edges: state
                ...
                .filter(|(m_id, e_id)| {
                    ...
                    mesh.cuts.get(e_id).is_some_and(|cut| !cut.is_dead)
                })
                .copied()
                .collect(),
        };
...
impl Command for ClearCutsCommand {
    fn execute(&self, state: &mut crate::State) -> Result<(), CommandError> {
        ...
        // Clear the cuts for each mesh in forward order
        edges_by_mesh.iter().for_each(|(mesh_id, edge_ids)| {
            if let Some(mesh) = state.meshes.get_mut(*mesh_id) {
                edge_ids.iter().for_each(|e_id| mesh.clear_cut(e_id, true));
            }
        });
        Ok(())
    }
    ...
    fn rollback(&self, state: &mut crate::State) -> Result<(), CommandError> {
        ...
        // Make the cuts for each mesh in reverse order
        edges_by_mesh.iter().for_each(|(mesh_id, edge_ids)| {
            if let Some(mesh) = state.meshes.get_mut(*mesh_id) {
                edge_ids.iter().rev().for_each(|e_id| mesh.make_cut(*e_id, true));
            }
        });
        Ok(())
    }
