use crate::cache::tool::select_paint::SelectPaintToolGPU;
use crate::gpu;

#[derive(Debug)]
pub struct ToolSelectPaintProgram {
    pipeline: wgpu::RenderPipeline,
}

impl ToolSelectPaintProgram {
    pub(super) fn new(ctx: &gpu::Context) -> Self {
        let shader = ctx
            .device
            .create_shader_module(wgpu::include_wgsl!("./shaders/tool_select_paint.wgsl"));
        Self {
            pipeline: ctx.device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                ...
                layout: Some(&ctx.device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                    ...
                    bind_group_layouts: &[
                        ...
                        &ctx.shared.bind_group_layouts.tool.select_paint,
                    ],
