use super::{GPUContext, GPUTexture};
pub struct GPUFrameBuffer {
    ...
    color_textures: Vec<GPUTexture>,
    depth_stencil_texture: Option<GPUTexture>,
}

impl GPUFrameBuffer {
    /// Initializes a frame buffer whose render call writes to the swapchain
    /// and then presents it to the screen.
    pub fn from_swapchain(ctx: &GPUContext) -> Self {
        Self {
            is_swapchain: true,
            color_textures: vec![], // Surface will provide the color texture
            depth_stencil_texture: Some(GPUTexture::create_depth_texture(ctx)),
        }
    }
    ...
    pub fn resize(&mut self, ctx: &GPUContext) {
        self.depth_stencil_texture = Some(GPUTexture::create_depth_texture(ctx))
    }
}
