...
pub fn import_gltf() -> Result<State, gltf::Error> {
                ...
                indices.chunks(3).for_each(|i| {
                    pp_mesh.add_face(
                        ...
                        &FaceDescriptor {
                            ...
                            m: material.index().map(id::MaterialId::from_usize).unwrap_or_default(),
                        },
                    );
                });
            };
    ...
    let images_map = &mut state.images;
    gltf.document.images().for_each(move |image| {
    ...
    let mut samplers = HashMap::<id::SamplerId, _>::new();
    gltf.document.samplers().for_each(|sampler| {
        samplers.insert(sampler.index().map(pp_core::id::SamplerId::from_usize).unwrap(), sampler);
    });

    gltf.document.textures().for_each(|texture| {
        let i = pp_core::id::TextureId::from_usize(texture.index());
        let s_id = texture.sampler().index().map(id::SamplerId::from_usize).unwrap_or_default();
        state.textures.insert(
            i,
            Texture {
                label: texture.name().unwrap_or(format!("{i:?}").as_str()).into(),
                image: id::ImageId::from_usize(texture.source().index()),
                sampler: samplers
                    .get(&s_id)
                    .map(|sampler| Sampler {
    ...
    gltf.document.materials().for_each(|material| {
        let i = material.index().map(pp_core::id::MaterialId::from_usize).unwrap_or_default();
        ...
        state.materials.insert(
            ...
            Material {
                ...
                base_color_texture: pbr_metallic_roughness
                    .base_color_texture()
                    .map(|info| id::TextureId::from_usize(info.texture().index()))
                    .unwrap_or_default(),
